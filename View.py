...
class View:
    ...
    def draw_welcome(self):
        ...
        #text_surface = self.font_small.render("Press Spacebar to continue", True, self.col_darkblue, self.BACKGR_COL)
        #text_rectangle = text_surface.get_rect()
    ...
    def draw_feedback (self, time, correct):
        if correct == True:
            text_surface = self.font_small.render ("You had the correct sequence of blocks", True, self.col_darkblue, self.BACKGR_COL)
        elif correct == False:
            text_surface = self.font_small.render ("You had an incorrect sequence of blocks", True, self.col_darkblue, self.BACKGR_COL)
        ...
        text_surface = self.font_small.render ("Your time was: " + str(time) + " seconds", True, self.col_darkblue, self.BACKGR_COL)
        text_rectangle = text_surface.get_rect()
    ...
    def draw_final(self, WMC = 0, avgTime = 1):
        text_surface = self.font_small.render("You have a WMC of " + str(WMC) + "s", True, self.col_darkblue, self.BACKGR_COL)
        text_rectangle = text_surface.get_rect()
        text_rectangle.center = (self.SCREEN_SIZE[0] / 2.0, 150)
        self.screen.blit(text_surface, text_rectangle)
